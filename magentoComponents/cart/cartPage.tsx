...
export default function CartPageView({
  ...
  return (
    <div className="mx-auto mb-[40px] max-w-[1440px] px-4">
      <h1 className="my-10 text-3xl font-bold">Shopping Cart Page</h1>
      {lineItems.lines?.length === 0 ? (
        <div>There are no items to display in the cart.</div>
      ) : (
        <div className="justify-between gap-10 md:flex">
          ...
          <div>
            <div className="my-6 h-fit min-w-72 border border-solid border-slate-300 p-4 md:my-0">
              ...
              <Link
                href={lineItems?.checkoutUrl}
                className="block rounded-full bg-lightBlack p-3 text-center text-[16px] font-medium text-white opacity-90 hover:opacity-100"
              >
                Proceed to Checkout
              </Link>
            </div>
            <div className="mt-[30px] border border-solid border-slate-300 p-4 md:mb-10">
