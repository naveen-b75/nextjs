...
function SortFilterItem({ item }: { item: SortFilterItemType }) {
  ...
  const DynamicTag = active ? 'p' : Link;
  const updatedSearchParams = new URLSearchParams({ sort: item.slug || '' });
  ...
  return (
    <li className="flex text-black dark:text-white" key={item.title}>
      <DynamicTag
        ...
        href={createUrl(pathname, updatedSearchParams)}
        className={clsx('w-full p-2 px-4 text-sm hover:bg-slate-200', {
          'bg-slate-200': active
        })}
      >
        {item.title}
