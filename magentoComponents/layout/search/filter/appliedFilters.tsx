...
const AppliedFilters: FC<AppliedFiltersProps> = ({
      ...
      Object.keys(params).forEach((key) => {
        ...
        if (Array.isArray(params[key])) {
          params[key].forEach((value: any) => {
            result.push(`${key.replace('[filter]', '')},${decodeURIComponent(value)}`);
          });
        } else {
          result.push(`${key.replace('[filter]', '')},${decodeURIComponent(params[key])}`);
        }
      });
  ...
  return (
    <>
      <div className="applied-filters">{appliedFiltersContent}</div>
