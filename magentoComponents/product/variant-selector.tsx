...
export function VariantSelector({
  ...
  setSelectedOptions: (value: { [key: string]: string }) => void;
}) {
  const [selectedOptions, setSelectedOptionsState] = useState<{ [key: string]: string }>({});
  ...
  const handleOptionClick = useCallback((optionName: string, value: string) => {
    setSelectedOptionsState((prevSelectedOptions) => {
      const updatedOptions = { ...prevSelectedOptions };
      // if (updatedOptions[optionName] === value) {
      //   delete updatedOptions[optionName]; // Remove the option if it is already selected
      // } else {
      //   updatedOptions[optionName] = value; // Add/Update the selected option
      // }
    ...
    <dl className="mb-4" key={i}>
      ...
      <dd className="flex flex-wrap gap-3">
        {option.values.map((value) => {
          ...
          const isActive = selectedOptions[optionId] == value.value_index;
          ...
          return (
            <button
              ...
              onClick={() => {
                ...
                handleOptionClick(optionId, value.value_index);
              }}
              title={`${option.label} ${value.store_label}${
                !isAvailableForSale ? ' (Out of Stock)' : ''
              }`}
              className={clsx(
                'flex min-w-[48px] items-center justify-center rounded-full border bg-neutral-100 px-2 pb-[3px] pt-[6px] text-sm dark:border-neutral-800 dark:bg-neutral-900',
                {
                  'cursor-default ring-2 ring-[#222222]': isActive,
                  'ring-1 ring-transparent hover:ring-[#222222] ': !isActive && isAvailableForSale,
