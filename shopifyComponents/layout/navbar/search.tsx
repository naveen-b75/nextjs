    ...
    <div className="min-w-[220px] lg:min-w-[350px]">
      <form onSubmit={onSubmit} className="relative w-full">
        <input
          ...
          autoComplete="off"
          value={searchParams?.get('q') || query}
                ...
                {results.aggregation.map((result: { label: string; value: string }, index) => (
                  <li className="p-2 hover:bg-gray-200" key={index}>
                    ...
                    <Link
                      ...
                      href={`/search?q=${query}&filter_category_uid=${result.label},${result.value}`}
                    >
                      {result.label}
                    </Link>
                  </li>
              ...
              <ul>
                ...
                {results.items.map(
                  (
                    result: {
                      title: string;
                      priceRange: { maxPrice: { currencyCode: string; amount: number } };
                      handle: string;
                    ...
                    <li key={index} className="mb-2 border-b pb-2">
                      <Link className="flex" href={`/product/${result.handle}`}>
                        <Image
                          alt="product"
                          className="border"
                        ...
                        <div className="ml-2">
                          ...
                          <Price
                            ...
                            amount={result?.priceRange?.maxPrice?.amount?.toString()}
                            currencyCode={result?.priceRange?.maxPrice?.currencyCode}
                          />
                        </div>
                      </Link>
                    </li>
