...
import { setReviewsData } from '../../../lib/magento';
function formatErrorMessage(err: Error): string {
  return JSON.stringify(err, Object.getOwnPropertyNames(err));
}
export async function POST(req: NextRequest): Promise<Response> {
  const { sku, nickname, ratings, summary, text } = await req.json();
  try {
    const response = await setReviewsData(sku, nickname, ratings, summary, text);
