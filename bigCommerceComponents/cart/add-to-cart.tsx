...
import { addItem } from 'bigCommerceComponents/cart/actions';
import LoadingDots from 'bigCommerceComponents/loading-dots';
...
function SubmitButton({
  ...
  const buttonClasses =
    'relative flex w-full items-center justify-center rounded-full bg-lightBlack p-4 tracking-wide text-white';
...
export function AddToCart({
  ...
  const variant = variants.find((variant: ProductVariant) =>
    variant.selectedOptions.every(
      (option) => option.value === selectedOption[option.name.toLowerCase()]
    )
  );
